...
class CheapDeck:
    ...
    def _handle_button_action(self, button_name: str) -> None:
        ...
        if button_config.get('enabled', True) is False:
            logger.info(f"{button_name} is disabled")
            return

        delay_ms = self._safe_int(button_config.get('delay_ms', 0), 0)
        repeat_count = self._safe_int(button_config.get('repeat_count', 1), 1) if button_config.get('repeat') else 1
        repeat_count = max(1, min(repeat_count, 50))

        for index in range(repeat_count):
            if delay_ms > 0:
                time.sleep(delay_ms / 1000.0)
            self._run_button_action(button_name, button_config)
            if index < repeat_count - 1:
                time.sleep(0.05)

    def _run_button_action(self, button_name: str, button_config: Dict[str, Any]) -> None:
        ...
        if action == 'key':
            ...
            keys = self._parse_hotkey(button_config.get('value') or button_config.get('shortcut', []))
            if keys:
            ...
            args = [path, "--new-tab"] if path else []
            if web:
                args.append(web)
            try:
                if args:
                ...
                logger.error(f"Could not find browser at {path}")
            except Exception as e:
                logger.error(f"Failed to open browser action: {e}")

        elif action == 'macro':
            self._run_macro(button_config.get('macro', ''))

        elif action == 'system':
            command = button_config.get('value') or button_config.get('description', '')
            if command:
                subprocess.Popen(command, shell=True)
                logger.info(f"Ran system command: {command}")

        elif action == 'media':
            key = button_config.get('value') or button_config.get('shortcut') or 'playpause'
            pyautogui.press(key)
            logger.info(f"Pressed media key: {key}")
    ...
    def _parse_hotkey(self, keys_config: Any) -> list[str]:
        if isinstance(keys_config, list):
            return [str(key).strip() for key in keys_config if str(key).strip()]
        if not keys_config:
            return []
        raw = str(keys_config).strip()
        try:
    ...
    def _run_macro(self, macro: str) -> None:
        for raw_line in str(macro).splitlines():
            line = raw_line.strip()
            ...
            value = value.strip()
            if command == 'delay':
                time.sleep(self._safe_int(value, 0) / 1000.0)
            elif command == 'type':
                pyautogui.write(value)
            elif command == 'press':
                pyautogui.press(value)
            elif command == 'hotkey':
                keys = self._parse_hotkey(value)
                if keys:
                    pyautogui.hotkey(*keys)
            elif command == 'open' and value:
                subprocess.Popen(value, shell=True)
            else:
                logger.warning(f"Unknown macro line: {line}")
    ...
    def _handle_slider_command(self, cmd: str) -> None:
        ...
        # Accept  formats: "slider 3 value: 95", "slider_3:95", "slider 3:95"
