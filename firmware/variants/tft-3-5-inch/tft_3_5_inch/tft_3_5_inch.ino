...
const int XP = 6, XM = A2, YP = A1, YM = 7;
  // Touch pins
const int TS_LEFT = 962 , TS_RT = 186, TS_TOP = 926 , TS_BOT = 188;  // Calibration values
...
const int ANALOG_PINS[] = {A15, A14, A13};
const int NUM_ANALOG_PINS = 3;
const int ANALOG_THRESHOLD = 1;  // Minimum change 

// Previous analog values for change detection
int prevAnalogValues[NUM_ANALOG_PINS] = {0};
unsigned long lastAnalogRead = 0;
const unsigned long ANALOG_READ_INTERVAL = 100;  
...
/**
 * Initialize analog pins for slider control
 */
...
void drawButton(Button &btn) {
  ...
  if (btn.hasIcon && btn.icon != NULL) {
    ...
    tft.drawRGBBitmap(iconX, iconY, btn.icon, btn.iconWidth, btn.iconHeight);
    
    // Draw label below icon with shadow for better readability
    tft.setTextSize(1);  // Smaller text when icon is present
    int16_t x1, y1;
    uint16_t w, h;
    tft.getTextBounds(btn.label, btn.x, textAreaY, &x1, &y1, &w, &h);
...
void drawSlider(Slider &s) {
  ...
  // Main thumb
  tft.fillRoundRect(s.x - 2, s.thumbY, thumbWidth, thumbHeight, 4, RED);
  // Highlight
  ...
  tft.drawRoundRect(labelX - 2, labelY - 2, w + 4, h + 4, 3, RED);
...
void drawHeader(String title) {
  ...
  // Draw separator line
  tft.drawFastHLine(0, headerHeight, tft.width(), RED);
  ...
  if (BUTTON_PAGES > 1) {
    ...
    for (int i = 0; i < BUTTON_PAGES; i++) {
      ...
      if (i == currentButtonPage) {
        ...
        tft.fillCircle(dotX, dotY, 5, RED);
  ...
  int dotY = 220;
    ...
    for (int i = 0; i < SLIDER_PAGES; i++) {
      ...
      if (i == currentSliderPage) {
        // Active page - larger filled circle with glow
        tft.fillCircle(dotX, dotY, 4, RED);
 ...
 */
void loop() {
  // Handle analog slider control
  handleAnalogSliders();

  // Read touch input
  TSPoint p = ts.getPoint();

  // Restore ALL touch pins (MEGA REQUIRED)
  pinMode(XP, OUTPUT);
  pinMode(XM, OUTPUT);
  pinMode(YP, OUTPUT);
  pinMode(YM, OUTPUT);
  ...
  if (p.z > MINPRESSURE && p.z < MAXPRESSURE) {
    ...
    // -------- RAW TOUCH (AXES SWAPPED) --------
    int tx = p.y;   // horizontal
    int ty = p.x;   // vertical

    // -------- MAP TO SCREEN --------
    int x = map(tx, TS_LEFT, TS_RT, 0, tft.width());
    int y = map(ty, TS_TOP,  TS_BOT, 0, tft.height());

    // -------- ROTATION FIX (setRotation(1)) --------
    // 90° clockwise
    y = tft.height() - y;
...
void handleMainMenuTouch(int x, int y) {
  ...
  // tft.fillCircle(x, y, 4, RED); // check touch 
